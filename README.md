 ...
 * Проблемы с рассылкой писем: иногда возникает ошибка `smtplib.SMTPServerDisconnected`. Для тестирования рекомендуется использовать свои SMTP-данные. Функционал почтовой рассылки работает корректно только с VPN. Конфигурация SMTP хранится в файле `.env`.
 ...
 * Отправка email: _smtplib, django.core.mail_  
...
1. Клонирование репозитория:
  ...
  ```bash
    git clone https://github.com/Keka17/BoardProject.git
...
4. Настройка переменных окружения
  ...
  Создайте файл `.env` в корневой папке и добавьте в него SMTP-настройки:
 
 ```bash
  EMAIL_HOST_USER=почта_для_рассылки

  EMAIL_HOST_PASSWORD=пароль_от_почтового_клиента
 ```
...
5. Применение миграций и запуск сервера
 ...
 ```bash
  ...
  python manage.py createsuperuser  # Создание админа для доступа к адним-панели
